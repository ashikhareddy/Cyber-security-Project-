## Overview

In Week 5, we started working on the actual network design and tried to turn our ideas into a proper diagram. The main focus was on making the visual part of the network better and getting feedback from our mentor.

## Progress

At the start of the week, Kunj and Masum created the first version of the network design. It was pretty basic at first, just simple boxes showing the components. We showed it to the mentor, and he told us to make it more detailed and realistic.

After that, they worked on improving the diagram by using proper icons and arranging it better. Masum also began working on the technical side by trying out IP configurations and setting up the network in Cisco Packet Tracer. Kunj kept improving the design layout.

Later in the week, we had a meeting where Masum explained the configurations he was working on, and Kunj showed the updated network design. Even though it was better, there were still parts that needed more work.

## Challenges

This week, we had a few problems. One big issue was that the first diagram was too simple and needed a lot of changes based on the mentor’s feedback. It took us some time to fix the design and include everything correctly.

Another major problem was communication. We had technical issues and missed mentor meetings for almost a week, and we didn’t tell the mentor about it. This caused a gap in support. Later, the unit coordinator contacted us and reminded us that without mentor meetings, we couldn’t submit the progress report in Week 6.

Because of this, we arranged an urgent meeting on Friday with both the mentor and unit coordinator to update them on what we’ve done so far.

## Reflection 

This week taught us how important it is to stay in regular contact with our mentor and plan properly. Missing meetings slowed us down and caused delays. We also learned that creating a good network diagram takes several rounds of improvements and feedback.

Even with these challenges, we managed to get back on track by organizing the meeting and continuing our work. From now on, we’ll make sure to communicate better and keep improving the design based on the feedback we get.

 ## configuration and network design in cisco packet tracer

 <img width="683" height="647" alt="server ip ping" src="https://github.com/user-attachments/assets/bacd2614-b06e-41f8-ab24-eaad5c880b3b" />
<img width="692" height="450" alt="ping test pc2" src="https://github.com/user-attachments/assets/22c91905-99c4-45d2-8d8b-00f3b2a3a924" />
<img width="688" height="673" alt="ping test pc1" src="https://github.com/user-attachments/assets/8b7672c6-a368-4ff0-890f-13f25935a2dc" />
<img width="682" height="665" alt="ping test od pc0" src="https://github.com/user-attachments/assets/eea9356a-1007-4429-b5f6-1ceea4912a73" />
<img width="692" height="667" alt="pc1 ip configuration" src="https://github.com/user-attachments/assets/462844cc-de3d-4684-b72b-121da0c48008" />
<img width="683" height="676" alt="pc0 ip configuration" src="https://github.com/user-attachments/assets/322422d6-a105-4f50-bcdb-8e967534e76e" />
<img width="683" height="398" alt="laptop0 ping test" src="https://github.com/user-attachments/assets/d6293406-eb1c-41c3-ae71-2135651688dc" />
<img width="673" height="665" alt="laptop0 ip configuration" src="https://github.com/user-attachments/assets/eceb116d-ff72-4dc0-b533-3e0b66b5bc09" />
<img width="681" height="627" alt="ip configuration pc2" src="https://github.com/user-attachments/assets/adb54d54-83e1-4516-a683-ce7db197de72" />
<img width="1240" height="687" alt="hospital A" src="https://github.com/user-attachments/assets/0f129a11-2309-4d65-9a23-6890b5f56c5f" />
<img width="1025" height="345" alt="entire network" src="https://github.com/user-attachments/assets/1ef25f5f-8b18-41a5-8f96-f7b2154b4341" />
<img width="682" height="586" alt="server ip config" src="https://github.com/user-attachments/assets/f44d0bd4-e011-4ee2-9434-a2e09f9cff73" />
<img width="685" height="476" alt="hospital router1" src="https://github.com/user-attachments/assets/7b093625-0ef4-42e2-859d-89c63162f083" />



Week 5 – Progress Update

In Week 5, I focused on designing and improving the network architecture for the healthcare system. My main contribution was creating detailed network diagrams that represent a secure and scalable hospital network.

Initially, I developed a basic hub-and-spoke topology connecting multiple hospitals (Hospital A, B, and C) to a central core router and data centre. This design ensures that all communication between hospitals is routed through a central system, improving control, monitoring, and security.

After that, I enhanced the design by adding key network components such as routers, switches, and servers to clearly show how data flows from hospital locations to the central data centre. This helped in improving the clarity and structure of the network.

I then designed a security layer model to demonstrate how data is protected at different levels. This includes Wi-Fi security (WPA2), VLAN segmentation, firewall protection, and encrypted data transmission before reaching the data centre. This layered approach ensures strong protection of sensitive healthcare information.

Finally, I created an advanced network diagram that includes VLAN segmentation for different user groups such as admin staff, doctors, and IoT/medical devices. I also integrated Cisco devices such as Router 2911, ASA 5506 firewall, and 2960 switch. This design improves network performance, access control, and overall security. VPN support was also included to enable secure communication between remote hospital sites.

Overall, my work in Week 5 shows the progression from a basic network structure to a more advanced and secure healthcare network design, focusing on scalability, segmentation, and data protection.

<img width="1098" height="730" alt="image" src="https://github.com/user-attachments/assets/25acbd49-cab4-4720-a5e6-cf190094e7db" />
<img width="611" height="874" alt="image" src="https://github.com/user-attachments/assets/2f61ffa3-d0eb-456c-a719-a497028ef594" />
<img width="951" height="781" alt="image" src="https://github.com/user-attachments/assets/521d2e74-a881-49de-9884-a16c431e05fb" />
<img width="1267" height="608" alt="image" src="https://github.com/user-attachments/assets/f0bd6694-3909-44ab-a173-4994e46ae7ef" />


